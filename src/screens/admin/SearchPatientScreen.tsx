...
import { TextInput, Card, Title, Paragraph, useTheme, Text, ActivityIndicator } from 'react-native-paper';
...
export default function SearchPatientScreen() {
  ...
  // Refetch every time this tab is focused (picks up newly enrolled patients)
  useFocusEffect(
    useCallback(() => {
      fetchPatients();
    }, [])
  ...
  const renderItem = ({ item }: { item: UserData }) => (
    <Card style={styles.card} mode="elevated">
      <Card.Content>
        ...
        {item.bloodGroup && <Paragraph>🩸 Blood Group: {item.bloodGroup}</Paragraph>}
        <View style={styles.idBadge}>
          <Text style={styles.patientId}>🪪 {item.patientId || item.id}</Text>
        </View>
      </Card.Content>
    </Card>
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <TextInput
        ...
        right={searchQuery ? <TextInput.Icon icon="close" onPress={() => handleSearch('')} /> : undefined}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>
              {searchQuery ? 'No patients match your search.' : 'No patients enrolled yet.'}
            </Text>
          }
...
const styles = StyleSheet.create({
  ...
  idBadge: {
    marginTop: 10, backgroundColor: '#e8f5e9',
